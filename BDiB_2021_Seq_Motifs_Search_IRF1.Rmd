...
```{r echo=FALSE}
getwd()
# "/homes/homedirs18/sghms/bms/shares/bdib/BDiB_2018_19/Session_09a_2019_03_05"
# setwd("/homedirs26/sghms/bms/users/anohturf/_BDiB/BDiB_2018_19")
```
...
Initially, let's tell R where to look for packages.  
```{r echo=FALSE}
lib.loc <- "/homes/homedirs18/sghms/bms/shares/bdib/BDiB_2018_19/R/packages_r_3_5_0"
...
# library(rGADEM, lib.loc=lib.loc)
# library(MotIV, lib.loc=lib.loc)
library(JASPAR2014, lib.loc=lib.loc)
# library(TFBSTools, lib.loc=lib.loc)
library(BSgenome.Hsapiens.UCSC.hg19, lib.loc=lib.loc)
```
...
```{r}
track.file <- "/homes/homedirs18/sghms/bms/shares/bdib/BDiB_2018_19/Session_09a_2019_03_05/Session_09a_R/wgEncodeRegTfbsClusteredV3.bed.gz"
...
irf1.b <- irf1 %>% 
  filter(expCount >= 4) %>% 
  filter(score >= 1000)
nrow(irf1.b) %>% cat("Number of remaining peaks:", .)
```

# Get DNA sequence for each peak  
```{r}
chr <- irf1.b$chrom
start <- irf1.b$chromStart
end <- irf1.b$chromEnd
...
What class of data hass been stored in the new list?
```{r}
